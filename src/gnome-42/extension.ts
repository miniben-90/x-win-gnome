...
let _dbus: Gio.DBusExportedObject | undefined = undefined;

function init() {
  /** Do nothing here! */
}

function enable(this: Extension) {
  _dbus = Gio.DBusExportedObject.wrapJSObject(DBUS_OBJECT, this);
  ...
  const active_window = global
    ...
    .find(
      (window_actor) =>
        ...
        _filterWindow(window_actor)
    );
  return JSON.stringify(_strcut_data(active_window));
}

function get_open_windows() {
  const open_windows = global.get_window_actors().filter(_filterWindow).map(_strcut_data);
  return JSON.stringify(open_windows);
}

function get_icon(window_id: number): string {
  const iconInfo = _get_icon(window_id);
  return JSON.stringify(iconInfo);
}

function _get_icon(window_id: number): XWinIcon {
  let icon: XWinIcon = {
    data: "",
    height: 0,
    width: 0,
...
function _strcut_data(
  ...
  if (meta_window && meta_window !== undefined && meta_window !== null) {
    ...
    const info = Object.assign(_get_process_info(process_id), {
...
function _get_memory_usage(pid: number): number {
...
function _get_process_info(process_id: number): {
  path: string;
  exec_name: string;
} {
  let process_info = {
